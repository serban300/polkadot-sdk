...
pub trait InspectMessageQueues {
	...
	/// `Self::take_messages()` will return only messages generated by said Runtime API.
	fn clear_messages();

	/// Take queued messages and their destinations.
	fn take_messages() -> Vec<(VersionedLocation, Vec<VersionedXcm<()>>)>;
}
...
impl InspectMessageQueues for Tuple {
	...
	fn take_messages() -> Vec<(VersionedLocation, Vec<VersionedXcm<()>>)> {
		...
		for_tuples!( #(
			messages.append(&mut Tuple::take_messages());
		)* );
